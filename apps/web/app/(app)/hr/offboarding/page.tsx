...
async function startOffboarding(formData: FormData) {
  ...
  const sql = db();
  const [emp] = await sql<{ org_id: string }[]>`select org_id from app.employees where id = ${employeeId}`;
  if (!emp) return;
...
export default async function OffboardingPage() {
  ...
  const records = await sql<{ employee_name: string; last_day: string | null; status: string; grace: string | null }[]>`
